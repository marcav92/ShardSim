...
for cross_shard_transaction_ratio in CROSS_SHARD_TRANSACTION_RATIOS:
    for file_number in range(AMOUNT_OF_FILES_PER_COMBINATION):
        for combination in POSSIBLE_COMBINATIONS:
            ...
            os.makedirs(f"{DATA_DIR}/{cross_shard_transaction_ratio}/{amount_leaf_shards}", exist_ok=True)
            os.makedirs(f"{STATIC_DATA_DIR}/{cross_shard_transaction_ratio}", exist_ok=True)

            file_prefix = f"{combination.shards}-{combination.children}-{combination.levels}-{file_number}"

            with open(f"{STATIC_DATA_DIR}/{cross_shard_transaction_ratio}/{file_prefix}-txs-file.tsv", "a") as f:
                f.writelines(raw_transaction_list)

            with open(f"{DATA_DIR}/{cross_shard_transaction_ratio}/{amount_leaf_shards}/{file_prefix}-txs_graph_nodes.txt", "a") as f:
            ...
            with open(f"{DATA_DIR}/{cross_shard_transaction_ratio}/{amount_leaf_shards}/{file_prefix}-labels.txt", "a") as f:
                for key in address_pools.keys():
